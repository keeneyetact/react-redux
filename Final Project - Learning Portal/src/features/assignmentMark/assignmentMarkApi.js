...
export const assignmentMarkApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    findAssignment: builder.query({
      query: ({ stdId, assignmentId }) =>
        `/assignmentMark?student_id=${stdId}&assignment_id=${assignmentId}`,
          ...
          if (markedAssignment?.id) {
            ...
            dispatch(
              apiSlice.util.updateQueryData(
                ...
                (draft) => {
                  return draft.map((d) =>
                    d.id == arg.id ? markedAssignment : d
                  );
                }
              )
            );
          ...
          if (submitedAssignment?.id) {
            ...
            dispatch(
              apiSlice.util.updateQueryData(
                "findAssignment",
                {
                  stdId: submitedAssignment.student_id,
                  assignmentId: submitedAssignment.assignment_id,
                },
